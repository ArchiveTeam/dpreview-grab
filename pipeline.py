...
LUA_SHA1 = get_hash(os.path.join(CWD, 'dpreview.lua'))
...
class WgetArgs(object):
    ...
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'thread':
                url = 'https://www.dpreview.com/forums/thread/' + item_value
                wget_args.extend(['--warc-header', 'dpreview-forum-thread: '+item_value])
                wget_args.extend(['--warc-header', 'dpreview-url: '+url])
                wget_args.append(url)
            elif item_type == 'post':
                url = 'https://www.dpreview.com/forums/post/' + item_value
                wget_args.extend(['--warc-header', 'dpreview-forum-post: '+item_value])
                wget_args.extend(['--warc-header', 'dpreview-url: '+url])
                wget_args.append(url)
            elif item_type == 'url':
                wget_args.extend(['--warc-header', 'dpreview-url: '+item_value])
...
project = Project(
    title='DPReview',
    project_html='''
        <img class="project-logo" alt="Project logo" src="https://wiki.archiveteam.org/images/thumb/2/25/Dpreview-icon.jpeg/600px-Dpreview-icon.jpeg" height="50px" title=""/>
        <h2>dpreview.com <span class="links"><a href="https://dpreview.com/">Website</a> &middot; <a href="http://tracker.archiveteam.org/dpreview/">Leaderboard</a></span></h2>
        <p>Archiving DPReview.</p>
    '''
)
